...
export async function getLocalTrending(location, tab) {
  // Replace trending with education search results
  // to restrict content to education purpose only
  // We use getLocalSearchResults directly to get the results
  const { results } = await getLocalSearchResults('education', {}, false)
...
export async function getLocalSearchResults(query, filters, safetyMode) {
  ...
  const response = await innertube.search(query + ' education', convertSearchFilters(filters))
...
export async function getLocalVideoInfo(id) {
  ...
  // Enforce education only by removing potentially distracting recommendations
  // Since we cannot easily verify the category of related videos in the local API without
  // significant overhead, we clear them to ensure the user stays focused.
  if (info.watch_next_feed) {
    info.watch_next_feed = []
  }

  // Strict Education Mode: Check Category
  // Local API (youtubei.js) provides 'category' in 'basic_info' or 'primary_info'
  const allowedCategories = ['Education', 'Science & Technology', 'Howto & Style']
  const videoCategory = info.basic_info.category || info.primary_info?.category?.text

  if (videoCategory && !allowedCategories.includes(videoCategory)) {
    throw new Error('This video is restricted to Educational purposes only.')
  }
  ...
  try {
    ...
    if (videosTab.current_tab?.endpoint.metadata.url?.endsWith('/videos')) {
      ...
      try {
        ...
        videos = playlist.items.map(parseLocalPlaylistVideo).filter(v => v)
      } catch (error) {
