...
export async function getInvidiousChannelShorts(channelId, sortBy, continuation) {
  // Education focus: Disable shorts
  return { continuation: null, videos: [] }
}
...
export async function invidiousGetVideoInformation(videoId) {
  ...
  // Invidious returns category as 'genre' string (e.g., "Education", "Gaming")
  const allowedGenres = ['Education', 'Science & Technology', 'Howto & Style']

  // Note: response.genre might be localized depending on the instance,
  // but standard usually returns English.
  // We check loosely to be safe, but default to blocking if unsure in strict mode.
  if (response.genre && !allowedGenres.includes(response.genre)) {
    // We can also check keywords if genre is missing or ambiguous,
    // but for now let's rely on the genre field.
    throw new Error('This video is restricted to Educational purposes only.')
  }

  // Replace recommended videos with education-focused search results
  // based on the current video's title to keep the user focused.
  if (response.title) {
    try {
      const educationalRecommendations = await getInvidiousSearchResults(response.title, 1, {
        sortBy: 'relevance',
        time: 'all',
        duration: 'all',
        type: 'video',
      ...
      if (educationalRecommendations && educationalRecommendations.length > 0) {
        response.recommendedVideos = educationalRecommendations.filter(item => item.type === 'video')
      } else {
        response.recommendedVideos = []
      }
    } catch (e) {
      console.warn('Failed to fetch educational recommendations', e)
      response.recommendedVideos = []
    }
  } else {
    response.recommendedVideos = []
...
export async function getInvidiousPopularFeed() {
  // Replace popular feed with education search results
  // to restrict content to education purpose only
  return await getInvidiousSearchResults('education', 1, {
    sortBy: 'relevance',
    time: 'all',
    duration: 'all',
    type: 'video',
...
export async function getInvidiousSearchResults(query, page, searchSettings) {
  ...
  results = results.filter((item) => {
    ...
    if (item.type === 'video' && item.lengthSeconds <= 60) {
      return false
    }
